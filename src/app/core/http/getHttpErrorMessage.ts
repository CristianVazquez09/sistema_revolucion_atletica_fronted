export function getHttpErrorMessage(err: any, fallback = 'Ocurrió un error'): string {
  ...
  switch (err.status) {
    ...
    case 400:
      return fromBody ?? 'Solicitud inválida (400).';
    case 401:
      return fromBody ?? 'Sesión expirada o credenciales inválidas (401).';
    case 403:
      return fromBody ?? 'No tienes permisos para esta acción (403).';
    case 404:
      return fromBody ?? 'Recurso no encontrado (404).';
    case 409:
    ...
    default:
      return fromBody ?? fromErr ?? fallback;
  }
}
