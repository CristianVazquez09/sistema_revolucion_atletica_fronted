...
export function calcularFechaFin(inicioISO: string, tiempo?: TiempoPlan | string | null): string {
  ...
  switch (t) {
    ...
    default:
      addMonths(1);
      break; // fallback razonable
  }
  return d.toISOString().slice(0, 10);
}

export function calcularTotal(precioPaquete: number, descuento = 0, costoInscripcion = 0): number {
  const n = Math.max(0, (precioPaquete || 0) + (costoInscripcion || 0) - (descuento || 0));
  return Math.round(n * 100) / 100;
}

export function dateLocalFromISO(iso: string): Date {
