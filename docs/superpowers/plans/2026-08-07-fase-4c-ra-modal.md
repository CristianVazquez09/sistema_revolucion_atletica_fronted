...
1. **Cierre con tecla Escape**: `usuarios-admin-modal` no tenía manejo de Escape (`ra-modal` lo agrega ahí como mejora nueva). **Corrección hallada en Task 4** (la investigación previa de este plan estaba equivocada en este punto): `producto-modal`/`stock-modal` SÍ tenían ya un listener manual (`window.addEventListener('keydown', handleEsc)` sin ninguna condición extra más que `e.key === 'Escape'`) — se retiró al adoptar `ra-modal` porque quedaba duplicado con su `HostListener` interno, sin perder ninguna guarda (no había ninguna).
...
6. **Blur del backdrop (hallado en la revisión final de rama)**: `usuarios-admin.html` tenía `backdrop-blur-[2px]`; las 2 instancias de `producto.html` no tenían blur. `ra-modal` estandariza con `backdrop-blur-[2px]` siempre — para `producto-modal`/`stock-modal` esto agrega un blur que no tenían antes, mismo tipo de convergencia cosmética menor que las anteriores, sin riesgo funcional.
